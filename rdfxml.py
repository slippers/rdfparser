...
class Element(object):
    def __init__(self, pfx_n, qn, attrs, p=None, base=None, qnames=True):
        pfx, n = pfx_n
...
class RDFParser(xml.sax.handler.ContentHandler):
    ...
    def startElementNS(self, n, q, a):
        if self.stack: e = Element(n, q, Attribs(a, self.qnames), self.stack[-1], qnames=self.qnames)
        else: e = Element(n, q, Attribs(a, self.qnames), base=self.base, qnames=self.qnames)
        e.xtext[0] += '<'+e.qname+((repr(e.attrs).strip() and repr(e.attrs)) or '')+'>'
        self.stack += [e]
    ...
    def nodeElement(self, e):
        ...
        if rdf.ID in e.attrs:
            e.subject = self.uri(urljoin(e.base, "#" + e[rdf.ID]))
        elif rdf.about in e.attrs:
            e.subject = self.uri(urljoin(e.base, e[rdf.about]))
        elif rdf.nodeID in e.attrs:
    ...
    def resourcePropertyElt(self, e):
       ...
       if rdf.ID in e.attrs:
           i = self.uri(urljoin(e.base, ('#' + e.attrs[rdf.ID])))
    ...
    def parseTypeLiteralOrOtherPropertyElt(self, e):
       ...
       self.triple(e.parent.subject, self.uri(e.URI), o)
       if rdf.ID in e.attrs:
           e.subject = i = self.uri(urljoin(e.base, ('#' + e.attrs[rdf.ID])))
           self.reify(i, e.parent.subject, self.uri(e.URI), o)
    ...
    def emptyPropertyElt(self, e):
        if e.attrs.keys() in ([], [rdf.ID]):
            ...
            if rdf.resource in e.attrs:
                r = self.uri(urljoin(e.base, e[rdf.resource]))
            elif rdf.nodeID in e.attrs:
...
class Sink(object):
    ...
    def triple(self, s, p, o):
        self.result += "%s %s %s .\n" % (s, p, o)
    def write(self):
        #print(self.result.rstrip().encode('utf-8'))
...
def parseRDF(s, base=None, sink=None):
    ...
    parser.parse(io.StringIO(s))
    return sink

def parseURI(uri, sink=None):
    with urlopen(uri) as f:
        return parseRDF(f.read().decode('utf-8'), base=uri , sink=sink)

if __name__=="__main__":
    if len(sys.argv) != 2:
        print(__doc__)
    else:
        parseURI(sys.argv[1]).write()
